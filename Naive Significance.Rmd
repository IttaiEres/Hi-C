...
```{r ROACH--most recent run}
#!/software/R-3.2-el6-x86_64/bin/R

#R file for creating tabulation DFs on midway.
...
#test <- fread("~/Desktop/full_tab_df_200kb", header=TRUE, stringsAsFactors = FALSE, data.table=FALSE)
...
signif.comparison <- function(chr, full_info, locus_size, percents=c(20, 15, 10, 5, 2.5, 1, 0.5, 0.1, 0.01)){
  ...
  #Get a data frame of the values above which we consider hits "significant" in the different distance bins (but genome-wide!)
  ...
  homer.sig <- fread(paste("/project2/gilad/ittai/HiC/roach/homerfiles/A-21792/sig_", locus_size, "kb/roachready/", chr, "_", locus_size, "kb_siglocs.txt", sep=""), header=TRUE, stringsAsFactors = FALSE, data.table=FALSE)
  ...
  if(nrow(homer.sig)==0){
    print("This homer file is empty!")
    ...
    #Get another column that is distance between the two loci, for subsetting down later on. The -1 shifts this number so that contacts between adjacent bins fall into my first binning done in basic_tabulation (another way to fix this is to change the bin range inclusions in basic_tabulation to include the upper but not the lower boundary).
    homer.sig$distance <- abs(homer.sig$loc1-homer.sig$loc2)-1
  ...
  print("Homer file successfully read-in and reformatted")  
  ...
  for(bin in rownames(sig.values)){
    for(percent in colnames(sig.values)){
      ...
      print(nrow(tmp.my.sigs))
      print(nrow(tmp.homer.sigs))


      if(nrow(tmp.my.sigs>0)){#Don't bother doing any of this if I don't have significant hits for the bin/percentage combo in question.      
      ...
      #Is it the rbind.fill that's running into errors later? Try assigning a 1-row df to empty options.
     } else{contacts.list[[paste(percent, bin, sep=", ")]] <- data.frame(chr=NA, dist.bin=NA, loc1_bin=NA, loc2_bin=NA, count=NA, percentile=(1-as.numeric(percent)*100), homer.hit=NA, my.hits.tot=0, homer.hits.tot=nrow(tmp.homer.sigs), overlap=0)}
    }
  }
...
do.it.all <- function(locus.bin.size, start=0, end=250000, by=5000, homer=TRUE, dataframe=FALSE, variable.list=c("chr1", "chr2", "chr3", "chr4", "chr5", "chr6", "chr7", "chr8", "chr9", "chr10", "chr11", "chr12", "chr13", "chr14", "chr15", "chr16", "chr17", "chr18", "chr19", "chr20", "chr21", "chr22", "chrX", "chrY")){
  if(dataframe==TRUE){
  coverage <- fread(paste("/project2/gilad/ittai/HiC/roach/coverage/coverage_A_", locus.bin.size, "kb_5kb.bed", sep=""), sep="\t", header=FALSE, stringsAsFactors=FALSE, data.table=FALSE) #Pull out coverage for the given size
  colnames(coverage) <- c("chr", "start", "end", "count") #Rename the columns of the coverage df to work with other functions
  
  positions <- fread("/project2/gilad/ittai/HiC/roach/positions/A_cis_pos_GW", sep="\t", header=FALSE, stringsAsFactors = FALSE, data.table = FALSE) #Load in the pos.df
  colnames(positions) <- c("chr", "pos1", "pos2")
  ...
  for(chromosome in variable.list){
    ...
    tmp.pos.df <- positions[positions$chr==chromosome,2:3] #Grab just the positions for the given chr. #fread(paste("/project2/gilad/ittai/HiC/roach/positions/cis_pos", substr(chromosome, 4, 5), sep=""), header=FALSE, stringsAsFactors=FALSE, data.table=FALSE)
  ...
  write.table(full_tab_df, file=paste("/project2/gilad/ittai/HiC/roach/test/full_tab_df_", locus.bin.size, "kb", sep=""), sep="\t", col.names=T, row.names=F, quote=F)
  ...
  full_tab_df <- fread("/project2/gilad/ittai/HiC/roach/test/full_tab_df_200kb", header=TRUE, stringsAsFactors=FALSE, data.table=FALSE)
  ...
  #Write this long-form df out into a separate file!
  write.table(full_homer_overlap_df, file=paste("/project2/gilad/ittai/HiC/roach/full_homer_overlap_dfs/A/full_homer_overlap_df.", locus.bin.size, "kb", sep=""), sep="\t", col.names=T, row.names=F, quote=F)
}}

##Actually do it!
#for(locus.bin.size in c(100, 150, 200)){
#  do.it.all(locus.bin.size = locus.bin.size, start=0, end=5000000, by=(locus.bin.size*1000))
#}

do.it.all(200, 0, 100000000, 200000)

setwd("/Users/ittaieres/Desktop/Hi-C/All_good_analysis/cis_only/cis_only_coverage")
The working directory was changed to /Users/ittaieres/Desktop/Hi-C/All_good_analysis/cis_only/cis_only_coverage inside a notebook chunk. The working directory will be reset when the chunk is finished running. Use the knitr root.dir option in the setup chunk to change the the working directory for notebook chunks.> full_coverage <- fread("coverage_10kb_5kb.bed", sep="\t", header=F, stringsAsFactors = FALSE)
