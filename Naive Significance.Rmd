...
```{r Sanity checking to make sure everything makes sense! Go through everything with chr 22 just to ensure}
...
tab.22 <- basic_tabulation(coverage.22, pos.22, 0, 35000000) #Tabulate all contact counts for chr22, just to check!
data.22 <- full.chrom.tabulator(coverage.22, pos.22, 0, 50000, 5000)


sig.22 <- sig.thresh.finder("chr22", data.22, my.ps)
...
signif.comparison <- function(homer.file, coverage.file, pos.file, chr){
...
```{r Functions used in call of main function: bin sum optimization, contact frequency tabulation, full chromosome contact tabulator, full chromosome visualizer, significance threshold finder, }
...
full.chrom.tabulator <- function(coverage_df, pos_df, start, end, by){
  ...
  for(distance in 1:length(distances)){ #Iterate through all the user-input distances
    ...
    tabulation.list[[distance]] <- data.frame(chr=unique(coverage_df$chr),  bin=distances[distance], loc1=tmp.table$pos1_bin, loc2=tmp.table$pos2_bin, count=tmp.table$count) #Put a data frame with all the info about contacts at this distance into the list
  }
  tabulation.df <- do.call(rbind, tabulation.list) #Combine all the lists into a single long-form dataframe.
  return(tabulation.df)
}

chrom.visualizer <- function(full_tab_df){
  ...
  for(bin in distances){ #Make and print individual histograms for each of the different distance bins!
    print(qplot(full_tab_df[full_tab_df$bin==bin,5], data=full_tab_df[full_tab_df$bin==bin,], geom="histogram", binwidth=1, xlab="Contact Count (Reads)", ylab="Frequency", main=paste("Histogram of Contact Counts for Pairs", bin/1000, "to", (bin/1000)+spacing, "kb apart,", unique(full_tab_df$chr))))
...
sig.thresh.finder <- function(chromosome, full_tab_df, percents){
