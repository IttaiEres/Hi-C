...
```{r Making heatmaps from homer sig ortho results; normalizing said results; linear modeling}
...
#Normalize data, plot mean vs variance by contact
normdata <- as.data.frame(pairs.Clow.it3.third)
means <- rowMeans(normdata)
vars <- apply(normdata, 1, var)
plot(means, vars, main="Mean vs. Variance in Homer-corrected contact frequency aross pairs")

#Try running a linear model!
species <- c(0,0,1,1,0,0,1,1)
design <- model.matrix(~1+species)

test <- normdata[sample(nrow(normdata), 100000),]
testfit <- lmFit(test, design)
testebayes <- eBayes(testfit)
testebayes$coefficients[,2]
testebayes$p.value[,2]
